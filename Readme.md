...
toBlockChain:         // Последний шаг регистрации, запись в блокчейн, возвращает userId и username
    user <string>     // Имя пользователя.
...
-   `GLS_ACCOUNT_NAME_PREFIX` - префикс для генерируемого имени аккаунта (`userId`)

-   `GLS_CREATOR_NAME` - имя аккаунта, выполняющего `newaccount`

-   `GLS_CREATOR_KEY` - приватный ключ аккаунта, выполняющего `newaccount`
